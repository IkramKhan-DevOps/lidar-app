...
class ScanLocalStorage {
    ...
    func finalizeScanFolder(tempFolderURL: URL, name: String?, coordinates: [[Double]]? = nil, coordinateTimestamps: [Double]? = nil, imageCount: Int, durationSeconds: Double? = nil, boundsSize: simd_float3? = nil, onComplete: ((URL, ScanMetadata) -> Void)? = nil) async -> (url: URL?, metadata: ScanMetadata?) {
        ...
        if let lastCoordinate = coordinates?.last, !coordinates!.isEmpty {
            ...
            os_log("🌍 [GEOCODING] Attempting reverse geocoding for coordinate: [%.6f, %.6f]", log: OSLog.default, type: .info, lastCoordinate[0], lastCoordinate[1])
            do {
                ...
                if let placemark = placemarks.first {
                    ...
                    os_log("✅ [GEOCODING] Successfully resolved location: %@", log: OSLog.default, type: .info, locationName!)
                } else {
                    locationName = "Location not found"
                    os_log("⚠️ [GEOCODING] No placemark found for coordinates", log: OSLog.default, type: .info)
                }
            } catch {
                locationName = "Geocoding failed"
                os_log("❌ [GEOCODING] Reverse geocoding failed: %@", log: OSLog.default, type: .error, error.localizedDescription)
            }
        } else {
            locationName = "No GPS coordinates"
            os_log("ℹ️ [GEOCODING] No GPS coordinates available for reverse geocoding. Coordinates count: %d", log: OSLog.default, type: .info, coordinates?.count ?? 0)
        }
        ...
        do {
            ...
            // Calculate bounds size for area and height calculations
            let boundsSizeString: String
            if let bounds = boundsSize {
                boundsSizeString = "[\(bounds.x), \(bounds.z), \(bounds.y)]"
            } else {
                boundsSizeString = "[0.0, 0.0, 0.0]"
            }
...
class ARScanner: NSObject {
    ...
    func startScan() {
        ...
        scanStartTime = Date()
        scanDuration = 0.0
        sceneBounds = BoundingBox() // Reset sceneBounds
        
        // Request location permission and start location manager for GPS coordinates
        locationManager.requestLocationPermission()
        locationManager.startUpdatingLocation()
        os_log("📍 [AR SCANNER] Started location manager for GPS tracking. Authorization status: %d", log: OSLog.default, type: .info, CLLocationManager.authorizationStatus().rawValue)
            ...
            Task {
                do {
                    let (finalFolderURL, metadata) = try await ScanLocalStorage.shared.finalizeScanFolder(
                        ...
                        boundsSize: self.sceneBounds.size
                    )
    ...
    private func exportDataAsZip() throws -> Data {
        ...
        do {
            ...
            if fileManager.fileExists(atPath: imagesFolderURL.path) {
                ...
                os_log("⚠️ [EXPORT ZIP] Images folder not found: %@", log: OSLog.default, imagesFolderURL.path)
            }
    ...
    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        ...
        if coordinates.count < 100 {
            ...
            if shouldCapture, let location = locationManager.latestLocation {
                ...
                if accuracy <= 50 { // More relaxed accuracy threshold
                    ...
                    if (-90...90).contains(latitude) && (-180...180).contains(longitude) {
                        ...
                        os_log("📍 [GPS] Captured coordinate: [%f, %f] with accuracy %.1fm at AR position [%f, %f, %f]", log: OSLog.default, type: .info, latitude, longitude, accuracy, currentPosition.x, currentPosition.y, currentPosition.z)
                    } else {
                        os_log("⚠️ [GPS] Invalid GPS coordinates detected: lat=%f, lon=%f", log: OSLog.default, type: .error, latitude, longitude)
                    }
                } else {
                    os_log("⚠️ [GPS] Poor GPS signal (accuracy: %.1fm). Skipping coordinate capture.", log: OSLog.default, type: .info, accuracy)
                }
            } else if shouldCapture {
                os_log("⚠️ [GPS] No location available for coordinate capture", log: OSLog.default, type: .info)
            }
        }
...
class ScanViewController: UIViewController, ARScannerDelegate {
    ...
    private func setupFlutterChannel() {
        // Get reference to the main Flutter method channel without overriding its handler
        if let flutterController = UIApplication.shared.delegate?.window??.rootViewController as? FlutterViewController {
            ...
            os_log("📱 [SCAN VIEW] Got reference to Flutter method channel", log: OSLog.default, type: .info)
        } else {
            os_log("Failed to get reference to Flutter method channel", log: OSLog.default, type: .error)
        }
    }
...
class ModelViewController: UIViewController, QLPreviewControllerDataSource, UIDocumentPickerDelegate {
    ...
    private func updateProcessButtonVisibility() {
        // Always show the Done button regardless of network connection
        processButton.isHidden = false
        // Clear any network-related status messages
        if statusLabel.text == "No internet connection. Please connect to Wi-Fi or cellular data." {
            statusLabel.text = ""
        }
    }
    ...
    @objc private func doneButtonTapped() {
        guard let folderURL = currentScanFolderURL else {
            showErrorAlert(message: "Scan folder not found. Please try again.")
            return
        }
        ...
        processButton.setTitle("Uploaded!", for: .normal)
    ...
    @available(iOS 16.0, *)
    @objc private func processTappedOld() {
        guard let scanFolderURL = currentScanFolderURL else {
...
class LocationManager: NSObject, CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // Always update latestLocation for AR scanning to use
        latestLocation = location
        
        // Only store in coordinates array if accuracy is good and we have space
        if coordinates.count < maxCoordinates && location.horizontalAccuracy <= 20.0 {
            ...
            os_log("📍 [LOCATION MANAGER] Captured coordinate: [%.6f, %.6f] with accuracy %.1fm at %@", log: OSLog.default, type: .info, coordinate[0], coordinate[1], location.horizontalAccuracy, location.timestamp.description)
        } else if location.horizontalAccuracy > 20.0 {
            os_log("⚠️ [LOCATION MANAGER] Poor accuracy location (%.1fm), updating latestLocation only", log: OSLog.default, type: .info, location.horizontalAccuracy)
        }
    }
